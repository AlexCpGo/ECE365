...
class Lab2(object):
    ...
    def print_smith_waterman_alignment(self,s1,s2,penalties) :
        ...
        j = 0
        # Alignment Part
        while (i<=CompLength and j<=CompLength):
            if s1[i] == s2[j]:
                alignment[0] = alignment[0] + s1[i]
                alignment[1] = alignment[1] + s2[j]
                i = i + 1
                j = j + 1
            elif s1[i] != s2[j]:
                if flag == 1: # s1 is shorter
                    if s2[j+1] == s1[i]:
    ...
    def find_exact_matches(self,list_of_reads,genome):
        ...
        Slice_length = len(list_of_reads[0])
        # Build the dictionary of genome
        genomeDict = {}
        chrOrdinal = 0
        l = 1 # l reprensent the ordinal number of base from one chromosome
        i = 0
        genome_new = genome.replace("\n","") # delete"\n" in the str of genome        
        while (i<len(genome_new)-Slice_length+1):
            if genome_new[i] == '>':
                chrOrdinal = chrOrdinal + 1
                N = str(chrOrdinal)
                i = i + 5 # Skip the line of >chrN

            elif genome_new[i] != '>':
                if genome_new[i+Slice_length-1] != '>':
                    Slice = genome_new[i:i+Slice_length]
                    l_char = str(l)
                    if Slice not in genomeDict.keys():
                        genomeDict[Slice]=['chr'+N+':'+l_char]
                    elif Slice in genomeDict.keys():
                        genomeDict[Slice].append('chr'+N+':'+l_char)
                    l = l + 1
                    i = i + 1
                elif genome_new[i+Slice_length-1] == '>':
                    # Jump to the next chr
                    i = i + Slice_length - 1
                    l = 1   


        matchList = []       
        # Match the reads with the dictionary of genome
        for n in range(len(list_of_reads)):
            if list_of_reads[n] in genomeDict.keys():
                matchList.append(genomeDict[list_of_reads[n]])
